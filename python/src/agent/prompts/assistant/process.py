...
def process(processors: list[AssistantTool], documents: list[Document]):
    return f"""
As a human with great insight, you're tasked with analyzing user queries and determining the necessary processing actions. Your response must always be in JSON format, containing a "_thinking" key and a "process" array of objects with 'type' and required parameters for each process type.
...
<process_types>
{'\n'.join([f'<process_type>{processor.name}: {processor.description}</process_type>' for processor in processors])}
</process_types>

<documents>
{'\n'.join([f'<document name="{document["metadata"]["name"]}" uuid="{document["metadata"]["uuid"]}" description="{document["metadata"]["description"]}">Preview: {document["text"].slice(0, 100)}...</document>' for document in documents])}
</documents>
